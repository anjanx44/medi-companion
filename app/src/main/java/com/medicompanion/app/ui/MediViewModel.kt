...
class MediViewModel(app: Application) : AndroidViewModel(app) {
    ...
    fun delete(id: String) {
        viewModelScope.launch {
            ...
            _msg.value = if (r.isSuccess) "Deleted" else "Deleting failed: ${r.exceptionOrNull()?.message}"
        }
    }

    fun update(entry: BpEntry) {
        viewModelScope.launch {
            val r = repo.update(entry)
            _msg.value = if (r.isSuccess) "Saved" else "Failed: ${r.exceptionOrNull()?.message}"
        }
    }
